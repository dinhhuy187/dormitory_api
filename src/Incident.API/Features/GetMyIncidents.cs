...
public static class GetMyIncidents
{
    public record Query(
        Guid? RoomId,
        string? Status,
    ...
    public class Endpoint : IEndpoint
    {
        ...
        {
            app.MapGet("api/incidents/me", async (
                ...
                CancellationToken ct) =>
            {
                ...
                var result = await handler.ExecuteAsync(userId, query, ct);
                return Results.Ok(result);
            })
            .WithTags("Incidents")
            .WithName("GetMyIncidents")
    ...
    {
        public async Task<ApiResponse<List<IncidentDto>>> ExecuteAsync(string userId, Query request, CancellationToken ct)
        {
            if (request.Page <= 0)
